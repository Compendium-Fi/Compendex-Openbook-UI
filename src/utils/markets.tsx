...
import { customMarketsList } from "./customMarkets";


export const USE_MARKETS: MarketInfo[] = customMarketsList.map((elm) => {
  return {
    ...elm,
    programId: new PublicKey(elm.programId),
    address: new PublicKey(elm.address),
    deprecated: false,
  };
});
...
export function MarketProvider({ marketAddress, setMarketAddress, children }: { marketAddress: string, setMarketAddress: (data: any) => void, children: any }) {
...
export function getTradePageUrl(marketAddress?: string) {
  if (!marketAddress) {
    const saved = typeof window !== "undefined" ? localStorage.getItem("marketAddress") : "8BnEgHoWFysVcuFFX7QztDmzuH8r5ZFvyP3sYwn1XTh6";
    if (saved) {
      marketAddress = JSON.parse(saved);
    }
    marketAddress = marketAddress || DEFAULT_MARKET?.address.toBase58() || "8BnEgHoWFysVcuFFX7QztDmzuH8r5ZFvyP3sYwn1XTh6";
  }
...
export function getMarketInfos(
  ...
  const customMarketsInfo = USE_MARKETS.map((m) => ({
    ...m,
